    ...
    try:
        ...
        payload = {
            ...
            "messages": [
                ...
                {
                    "role": "user",
                    "content": f"""ข้อเสนอโครงการวิจัย: {proposal_text}\n\nเกณฑ์การพิจารณา: {criterion['name']}\n{prompt}\nโปรดให้คะแนน (0-10) และข้อเสนอแนะในบรรทัดเดียว โดยขึ้นต้นด้วย "คะแนน: X" ตามด้วยข้อเสนอแนะ"""
                }
            ],
            "temperature": 0.5,
            "max_tokens": 500
...
page = st.sidebar.radio("Go to", ["Evaluate Proposal", "Edit Rubric"])

def rubric_editor():
    rubric_name = st.session_state.selected_rubric if "selected_rubric" in st.session_state else "No Rubric Selected"
    st.markdown(
        f"<h2 style='font-weight:bold; color:#4B2E83; margin-bottom: 0.5rem;'>{rubric_name}</h2>",
    ...
    with st.expander("➕ Add New Criterion", expanded=False):
        ...
        new_prompt = st.text_area("Prompt (include all instructions here)", key="new_crit_prompt", height=150)
    ...
    for i, c in enumerate(criteria):
        with st.expander(f"{c['name']} (Weight: {c.get('weight',1.0):.2f})", expanded=False):
            ...
            prompt = st.text_area("Prompt", value=c["prompt"], key=f"prompt_{i}", height=150)
...
def proposal_evaluation():
    st.title("📝 Proposal Evaluation (Weighted Rubric)")
